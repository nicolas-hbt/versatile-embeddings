...
def get_parameter():
    ...
    parser.add_argument('-use_proto', default=0, type=int, help="whether to use the proto KG")
...
if __name__ == '__main__':
    ...
    if args.pipeline == 'both' or args.pipeline == 'test':
        ...
        for epoch in epochs2test:
            print("Epoch n°", epoch)
            if model == 'ConvE':
                model_path = directory + "instance__dim=" +str(args.dim) +\
        ...
        "_feat_drop="+str(args.feat_drop) + "_bs="+str(args.batch_size) + "__epoch="+str(epoch) + ".pt"
                ...
                model_path = directory + "instance__dim="+str(args.dim_e) + \
        ...
        "_lr="+str(args.lr) + "_reg="+str(args.reg) + "_bs="+str(args.batch_size) + "__epoch="+str(epoch) + ".pt"
            ...
            if True:
                if True:
                    ...
                    if model == 'ConvE':
                        df_valid.to_csv ('results/' + dataset.name + "/" + args.model + '/' + '['+ date_today + ']'+ args.model + '-valid_results-' + \
                            args.dataset + '-' + '-dim=' + \
                ...
                "-feat_drop="+str(args.feat_drop) + '-bs=' + str(args.batch_size) + '.csv')
                        with open('results/' + dataset.name + "/" + args.model + '/' + '['+ date_today + ']' + args.model + '-valid_results-' + \
                ...
                "-hid_dropout2="+str(args.hidden_dropout2) + '-bs=' + str(args.batch_size) + '.json', 'w') as fp:
                        ...
                        df_valid.to_csv ('results/' + dataset.name + "/" + args.model + '/' + '['+ date_today + ']'+ args.model + '-valid_results-' + args.dataset + '-' + model + '-dim=' + \
                str(args.dim) + '-lr=' + str(args.lr) + "-reg="+str(args.reg) + '-bs=' + str(args.batch_size) + '.csv')
                        with open('results/' + dataset.name + "/" + args.model + '/' '['+ date_today + ']'+ args.model + '-valid_results-' + args.dataset + '-' + model + '-dim=' + \
        ...
        if model == 'ConvE':
            best_model_path = directory + "instance__dim=" +str(args.dim) +\
    "_lr="+str(args.lr) + "_input_drop="+str(args.input_drop) + "_hid_dropout="+str(args.hidden_drop) + \
    "_feat_drop="+str(args.feat_drop) + "_bs="+str(args.batch_size) + "__epoch="+str(best_epoch) + ".pt"
            model_inst = ConvE(dataset.num_ent(), dataset.num_rel(), args, args.device)

        elif model == 'TuckER':
            best_model_path = directory + "instance__dim="+str(args.dim_e) + \
    "_lr="+str(args.lr) + "_input_drop="+str(args.input_dropout) + "_hid_dropout1="+str(args.hidden_dropout1) + \
    "_hid_dropout2="+str(args.hidden_dropout2) + "_bs="+str(args.batch_size) + \
    ...
    "_lr="+str(args.lr) + "_reg="+str(args.reg) + "_bs="+str(args.batch_size) + "__epoch="+str(best_epoch) + ".pt"
        ...
        if model == 'ConvE':
            df_test.to_csv ('results/' + dataset.name + "/" + args.model + '/' + '['+ date_today + ']'+ args.model + '-best-epoch_results-' + \
                args.dataset + '-' + '-dim=' + \
        str(args.dim) + '-lr=' + str(args.lr) + "-input_drop="+str(args.input_drop) + "-hid_dropout="+str(args.hidden_drop) + \
        "-feat_drop="+str(args.feat_drop) + '-bs=' + str(args.batch_size) + '.csv')
            with open('results/' + dataset.name + "/" + args.model + '/' + '['+ date_today + ']'+ args.model + '-best-epoch_results-' + \
                args.dataset + '-' + model + '-dim=' + \
        str(args.dim) + '-lr=' + str(args.lr) + "-input_drop="+str(args.input_drop) + "-hid_dropout="+str(args.hidden_drop) + \
        "-feat_drop="+str(args.feat_drop) + '-bs=' + str(args.batch_size) + '.json', 'w') as fp:
        ...
        "-hid_dropout2="+str(args.hidden_dropout2) + '-bs=' + str(args.batch_size) + '.json', 'w') as fp:
            ...
            df_test.to_csv ('results/' + dataset.name + "/" + args.model + '/' + '['+ date_today + ']'+ args.model + '-best-epoch_results-' + args.dataset + '-' + model + '-dim=' + \
        str(args.dim) + '-lr=' + str(args.lr) + "-reg="+str(args.reg) + '-bs=' + str(args.batch_size) + '.csv')
            with open('results/' + dataset.name + "/" + args.model + '/' '['+ date_today + ']'+ args.model + '-best-epoch_results-' + args.dataset + '-' + model + '-dim=' + \
        str(args.dim) + '-lr=' + str(args.lr) + "-reg="+str(args.reg) + '-bs=' + str(args.batch_size) + '.json', 'w') as fp:
                json.dump(best_ep_results, fp)